...
- `results/*_test_outputs.csv` generated by `benchmarking/cross_site.py` when
...
- `results/*_pipeline_abbreviations.csv` maps pipeline abbrev (P0001, ...) to
...
Library usage from a notebook or script:
    ...
    df = pd.read_csv("results/cross_site_full_test_outputs.csv")
    abbrev = pd.read_csv("results/cross_site_full_pipeline_abbreviations.csv")
...
def merge_pipeline_outputs(
    ...
    *,
    ...
    if len(merged) != len(df_a) or len(merged) != len(df_b):
        raise ValueError("Pipeline A and B do not share identical test samples")
